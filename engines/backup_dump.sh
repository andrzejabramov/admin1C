...
BACKUP_DIR="/var/backups/1c/dump"
PG_HOST="10.129.0.27"
PG_USER="postgres"
...
mkdir -p "$BACKUP_DIR"
echo "[$(date)] Начало резервного копирования (формат .dump)..."
...
while IFS=$'\t' read -r uuid name; do
    ...
    echo "  → $name"
    PGPASSFILE="$PG_PASSFILE" pg_dump -h "$PG_HOST" -U "$PG_USER" -d "$name" -Fc -f "$DUMP_FILE"
done <<< "$BACKUP_LIST"
