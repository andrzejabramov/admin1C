...
| №   | Правило                              | Причина                                                           |
| --- | ------------------------------------ | ----------------------------------------------------------------- |
| 1   | Всегда указывайте `--cluster=<uuid>` | Без него команды не работают                                      |
| 2   | Удаление ИБ необратимо               | Перед `drop` проверяйте `uuid` через `summary list`               |
| 3   | Пароли БД — только через файл        | `--db-pwd-file`, никогда в командной строке                       |
| 4   | Сессии ≠ соединения                  | Завершайте через `session terminate`, а не `connection terminate` |
| 5   | Выполняйте от `usr1cv8`              | Иначе — ошибка доступа                                            |

🔒 Интеграция с проектом  
В гибридной архитектуре rac вызывается из Bash-движков (engines/):
...
Преимущества:
Чистый Python-код (без парсинга вывода rac)
Изоляция привилегированных операций
Простая отладка каждого движка отдельно
EOF
