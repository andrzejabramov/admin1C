...
![Промпты для администрирования 1С](https://github.com/andrzejabramov/admin1C/blob/main/img/prompts_admin1c.png)
...
# 🤖 Промпт для генерации сервиса бэкапов

Контекст проекта:

- ОС: Ubuntu 18.04 LTS
- 1С:Предприятие 8.3.27.1989
- Кластер: один кластер, пользователь владельца — usr1cv8
- PostgreSQL: 11+, подключение через .pgpass (/home/usr1cv8/.pgpass) с правами 600
- Архитектура: гибридная (Python 3.11 для управления + Bash для низкоуровневых операций)
- Структура проекта: /opt/1cv8/scripts/{admin1c.py, lib/config.py, engines/, docs/}
- Требования безопасности: никаких паролей в коде, все привилегированные операции через sudo в изолированных Bash-скриптах

Задача: Создать сервис резервного копирования информационных баз 1С с поддержкой двух форматов.

Требования к реализации:

1. Единая точка входа:

   - Расширить admin1c.py новой подкомандой: `python admin1c.py backup --format {dump|sql} [--all | --ib <имя>] [--dry-run]`
   - Использовать существующую функцию `run_engine()` для вызова движка
   - Валидация аргументов через argparse

2. Bash-движок (engines/backup.sh):

   - Единый скрипт для обоих форматов (НЕ два отдельных файла)
   - Поддержка параметров: `--format dump|sql`, `--ib <имя_иб>`, `--dry-run`
   - Автоматическое определение параметров БД через `rac infobase summary list` (db-server, db-name, db-user)
   - Форматы:
     - `dump` → pg_dump -Fc -f backup.dump (кастомный формат, быстрое восстановление, НЕ блокирует ИБ)
     - `sql` → pg_dump | gzip > backup.sql.gz (сжатый текстовый дамп, переносимость, МОЖЕТ блокировать ИБ на время дампа)
   - Структура хранения: /var/backups/1c/<имя*иб>/<дата*время>/backup.{dump|sql.gz}
   - Логирование в /var/log/1c-admin/backup.log через функцию log() (вывод в stderr + запись через sudo tee)
   - Обработка ошибок через функцию error() с exit 1
   - Проверка свободного места перед бэкапом (минимум 2× размер ИБ)
   - Очистка временных файлов при прерывании (trap на EXIT)

3. Конфигурация (lib/config.py):

   - Добавить константы: BACKUP_ROOT="/var/backups/1c", BACKUP_LOG="/var/log/1c-admin/backup.log"
   - Функция get_backup_dir(ib_name) → Path к директории бэкапа

4. Документация (docs/backup.md):

   - Архитектурная схема компонентов (Python CLI → Bash-движок → pg_dump)
   - Таблица сравнения форматов (блокировка ИБ, скорость, размер, восстановление)
   - Алгоритм работы с диаграммой Mermaid (проверка места → определение параметров БД → выбор формата → выполнение дампа)
   - Примеры использования через единую точку входа
   - Диагностика: проверка .pgpass, права на /var/backups, логи
   - Важное замечание: .sql.gz МОЖЕТ блокировать ИБ на время дампа (особенно для больших баз), .dump — нет

5. Критические правила безопасности:

   - Пароль БД НИКОГДА не передаётся в аргументах командной строки
   - Использовать только .pgpass с правами 600
   - Все операции с БД выполняются от имени usr1cv8 через sudo -u usr1cv8
   - Проверка существования ИБ перед бэкапом (защита от опечаток)

6. Интеграция с существующими компонентами:
   - Использовать список ИБ из ib_list.conf (load_ib_list())
   - Для --all — перебор всех ИБ из списка с паузой 5 сек между базами
   - Совместимость с будущим сервисом выгрузки в облако (структура каталогов должна соответствовать ожиданиям cloud_upload.sh)

Вывод: предоставить 4 файла:

1. Дополненный admin1c.py (с подкомандой backup)
2. engines/backup.sh (единый скрипт с поддержкой --format)
3. Дополненный lib/config.py (константы и функции для бэкапов)
4. Полный текст docs/backup.md в формате Markdown с диаграммой Mermaid и таблицами

## 💡 Как использовать этот промпт

- Скопируйте промпт целиком (включая контекст проекта)
- Вставьте в новый чат с ИИ-ассистентом (мной или другим)

## Нюансы текущей реализации сервиса:

Дополнительно: учти, что на сервере уже существуют скрипты
engines/backup_dump.sh и engines/backup_sql.sh — их нужно заменить
единым движком, но сохрани логику работы с pg_dump из этих файлов.
