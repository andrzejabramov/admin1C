...
| [`rm`](../rm/docs/rm.md)                | Ручное удаление бэкапов по временной метке, дате или глобально (`--all`). Требует `--confirm` для реальных операций.  | `ib1c rm --ib ИБ [--timestamp МЕТКА \| --older-than ДАТА \| --all] [--dry-run] [--confirm]`        |
...
- [rm.md](../rm/docs/rm.md) — детали работы с бэкапами
